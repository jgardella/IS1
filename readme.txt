This file explains the different code in this project. There is C++ code and MATLAB code. For a more readable introduction to the code base, check out `IS1-steps.md`.
...
(extractChannelsFromSlices.m)
    ...
    It also takes a graph file name, and will save a MAT file containing a graph representation of the channels to that file.
    To create a mesh from the csv this function creates, pass it to skel_main.cpp.
    To display the graph mat this function creates, pass it to displayGraph.m.
    To create a mesh of the shortest path in the graph, pass the graph mat to saveShortestPath.m.

(displayGraph.m)
    Displays a graph mat generated by extractChannelsFromSlices, with the shortest path through the graph outlined in green.

(saveShorestPath.m)
    Generates a ply representing the shortest path through the given graph file.
...
and the C++ code. The parameters are as follows:
    ...
    <graphFile> name of graph file to save graph result to
